...
- [ ] Add `neighbours()` and `morph()` verbs to the reference API
      (`packages/elo-dictionary/src/compression_dictionary/api.py`) — the conformance
      suite cannot cover a channel the reference does not implement; `INDEX.json` lists
      both as `uncovered`.
...
- [ ] Add `epa` + `neighbours` contract files — they are the two channels with no
      `*.names.json`, which is what forces native consumers to hand-decode geometry.
...
- [x] 2026-09-24 **Conformance suite** — `compression_dictionary.conformance`: 26 vectors
      across 15 verbs, generated by the reference, shipped in every bundle under
      `conformance/`, reference passes 26/26. First run caught two suite-vs-itself bugs
      (set ordering in `wordclass.classes`; `require` verb missing from `Dictionary`).
